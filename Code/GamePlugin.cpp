...
#include "Components/PlayerComponentFlyby.h"
...
bool CGamePlugin::OnClientConnectionReceived(int channelId, bool bIsReset)
{
	...
	if (CSpawnPointComponent::TryGetFirstSpawnPointTransform(spawnTransform))
	{
		spawnParams.vPosition = spawnTransform.GetTranslation();
		spawnParams.qRotation = Quat(spawnTransform);
		// CryLog("[GamePlugin] Spawning player at SpawnPoint pos=(%.2f, %.2f, %.2f)",
		// 	spawnParams.vPosition.x, spawnParams.vPosition.y, spawnParams.vPosition.z);
	}
	else
	{
		CryLog("[GamePlugin] No SpawnPoint found during player spawn. Using default entity spawn transform.");
	}
	...
	if (IEntity* pPlayerEntity = gEnv->pEntitySystem->SpawnEntity(spawnParams))
	{
		...
		CPlayerComponentFlyby* pPlayer = pPlayerEntity->GetOrCreateComponentClass<CPlayerComponentFlyby>();
...
bool CGamePlugin::OnClientReadyForGameplay(int channelId, bool bIsReset)
{
	...
	if (it != m_players.end())
	{
		...
		{
			if (CPlayerComponentFlyby* pPlayer = pPlayerEntity->GetComponent<CPlayerComponentFlyby>())
			{
				// pPlayer->OnReadyForGameplayOnServer(); // Not implemented for flyby
			}
		}
	}
