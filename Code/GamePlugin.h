...
public:
	...
	template<typename T>
	void IterateOverPlayers(std::function<void(T& player)> func) const;
...
template<typename T>
void CGamePlugin::IterateOverPlayers(std::function<void(T&)> func) const
{
	for (const auto& playerPair : m_players)
	{
		if (IEntity* pPlayerEntity = gEnv->pEntitySystem->GetEntity(playerPair.second))
		{
			if (T* pPlayer = pPlayerEntity->GetComponent<T>())
			{
				func(*pPlayer);
			}
		}
